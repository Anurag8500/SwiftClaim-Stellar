...
export default function VeteranSend({
  ...
  const [localAmount, localSetAmount] = useState('50')
  ...
  const [localLivePrice, setLocalLivePrice] = useState<number | null>(null)

  const livePrice = propLivePrice !== undefined ? propLivePrice : localLivePrice
  ...
  // Fetch live price when asset changes (only if livePrice is not passed by parent)
  useEffect(() => {
    if (propLivePrice !== undefined) {
      return
    }

    if (selectedAsset === 'USDC') {
      setLocalLivePrice(1.0)
      return
    }
    ...
    fetch(`/api/price?asset=${selectedAsset}`)
      ...
      .then(data => {
        if (!cancelled && data.price) {
          setLocalLivePrice(data.price)
        }
      })
      .catch(() => {
        if (!cancelled) setLocalLivePrice(null)
      })

    return () => { cancelled = true }
  }, [selectedAsset, propLivePrice])
    ...
    return (
      <div className="space-y-6 text-center">
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          onClick={handleReset}
          className="w-full rounded-2xl bg-gradient-to-r from-[#ff5500] to-[#ff7700] hover:from-[#e04b00] hover:to-[#ff5500] px-6 py-4 font-bold text-white shadow-lg shadow-orange-500/25 border border-orange-500/20 transition-all"
        >
  ...
  return (
    <div className="space-y-6">
      ...
      {!isParentControlled && !isBelowMinimum && estimatedFiatValue > 0 && (
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-4 space-y-1">
          ...
          <div className="border-t border-zinc-800 pt-1 mt-1 flex justify-between text-sm font-semibold">
            ...
            <span className="text-orange-500 font-semibold font-mono">
              {totalCost !== null
